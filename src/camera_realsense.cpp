...
std::tuple<RealSenseProperties, bool, bool> CameraRealSense::initialize(sdk::ResourceConfig cfg) {
    ...
    std::tie(pipe, props) =
        startPipeline(disableDepth, width, height, disableColor, width, height, serial_number);
    ...
    std::thread cameraThread(frameLoop, pipe, std::ref(ready), device_, props.depthScaleMm,
...
std::tuple<rs2::pipeline, RealSenseProperties> startPipeline(
    bool disableDepth, int depthWidth, int depthHeight, bool disableColor, int colorWidth,
    int colorHeight, const std::string &target_serial_number) {
    rs2::context ctx;
    ...
    for (auto &&dev : devices) {
        ...
        VIAM_SDK_LOG(info) << "found available RealSense device with serial number: "
...
void on_device_reconnect(rs2::event_information &info, rs2::pipeline pipeline,
    ...
    if (device == nullptr) {
        throw std::runtime_error(
            "no device info to reconnect to. RealSense device "
    ...
    for (auto &&new_dev : info.get_new_devices()) {
        if (info.was_added(new_dev)) {  // assures we're checking a device that was
            ...
            if (target_serial_number.empty()) {
                ...
                VIAM_SDK_LOG(info)
                    << "[on_device_reconnect] A new RealSense device (S/N: " << new_dev_serial
                ...
                if (new_dev_serial == target_serial_number) {
                    VIAM_SDK_LOG(info) << "[on_device_reconnect] Configured device with S/N "
    ...
    try {
        std::tie(pipeline, props) = startPipeline(
            device->disableDepth, device->depthWidth, device->depthHeight, device->disableColor,
            device->colorWidth, device->colorHeight, target_serial_number);
    } catch (const std::exception &e) {
    ...
    std::thread cameraThread(frameLoop, pipeline, std::ref(ready), device, props.depthScaleMm,
        ...
        if (const sdk::ProtoList *sensors_list = attrs["sensors"].get<sdk::ProtoList>()) {
            if (sensors_list->empty()) {
                throw std::invalid_argument(
                    "sensors field cannot be empty, must list "
