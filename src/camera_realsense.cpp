...
CameraRealSense::initialize(sdk::ResourceConfig cfg) {
  if (device_ != nullptr) {
  ...
  if (attrs.count("serial_number")) {
    if (const std::string *serial_val =
            attrs["serial_number"].get<std::string>()) {
  ...
  if (attrs.count("sensors")) {
    if (const sdk::ProtoList *sensor_list_ptr =
            attrs["sensors"].get<sdk::ProtoList>()) {
  ...
  VIAM_SDK_LOG(debug) << "disableDepth: " << disableDepth
                      << " disableColor: " << disableColor << " sensors size "
                      << sensors.size();

  // DeviceProperties context also holds a bool that can stop the thread if
  // device gets disconnected
  std::shared_ptr<DeviceProperties> newDevice =
      std::make_shared<DeviceProperties>(width, height, disableColor, width,
                                         height, disableDepth,
                                         this->latest_frames_);
  ...
  std::tie(pipe, props) = startPipeline(
      disableDepth, width, height, disableColor, width, height, serial_number);
  ...
  std::thread cameraThread(frameLoop, pipe, std::ref(ready), device_,
                           props.depthScaleMm, std::ref(this->latest_frames_));
...
CameraRealSense::CameraRealSense(sdk::Dependencies deps,
                                 sdk::ResourceConfig cfg)
    : Camera(cfg.name()) {
  RealSenseProperties props;
  bool disableColor;
  bool disableDepth;
  try {
    ...
    throw std::runtime_error("failed to initialize realsense: " +
                             std::string(e.what()));
  }
  this->props_ = props;
...
void CameraRealSense::reconfigure(const sdk::Dependencies &deps,
  ...
  try {
    ...
    throw std::runtime_error("failed to reconfigure realsense: " +
                             std::string(e.what()));
  }
  this->props_ = props;
  this->disableColor_ = disableColor;
...
sdk::Camera::raw_image
CameraRealSense::get_image(std::string mime_type,
  ...
  if (this->props_.mainSensor.compare("color") == 0) {
    ...
    if (mime_type.compare("image/png") == 0 ||
      ...
      response = encodeColorPNGToResponse(
          (const void *)latestColorFrame.get_data(), this->props_.color.width,
          this->props_.color.height);
  ...
  if (debug_enabled) {
    ...
    VIAM_SDK_LOG(debug) << "[get_image]  total:           " << duration.count()
                        << "ms\n";
  }
...
sdk::Camera::image_collection CameraRealSense::get_images() {
  ...
  response.metadata.captured_at = sdk::time_pt{
  ...
  if (debug_enabled) {
    ...
    auto duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
    VIAM_SDK_LOG(debug) << "[get_images]  total:           " << duration.count()
                        << "ms\n";
  }
...
CameraRealSense::get_point_cloud(std::string mime_type,
                                 const sdk::ProtoStruct &extra) {
  std::chrono::time_point<std::chrono::high_resolution_clock> start;
  if (debug_enabled) {
    start = std::chrono::high_resolution_clock::now();
  ...
  if (debug_enabled) {
    ...
    auto duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
    VIAM_SDK_LOG(debug) << "[get_point_cloud]  total:           "
                        << duration.count() << "ms\n";
  }
...
CameraRealSense::get_geometries(const sdk::ProtoStruct &extra) {
...
void frameLoop(rs2::pipeline pipeline, std::promise<void> &ready,
  ...
  // start the callback function that will look for camera disconnects and
  // reconnects. on reconnects, it will close and restart the pipeline and
  // thread.
  rs2::context ctx;
  ctx.set_devices_changed_callback([&](rs2::event_information &info) {
    on_device_reconnect(info, pipeline, deviceProps);
  });
  ...
  while (true) {
    {
      ...
      if (!deviceProps->shouldRun) {
        pipeline.stop();
        VIAM_SDK_LOG(info)
            << "[frameLoop] pipeline stopped, exiting frame loop";
        break;
      }
    }
    ...
    */
    ...
    if (!succ) {
      if (debug_enabled) {
        VIAM_SDK_LOG(error)
            << "[frameLoop] could not get frames from realsense after "
            << timeoutMillis << "ms";
      }
    ...
    if (debug_enabled) {
      ...
      auto duration =
          std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
      VIAM_SDK_LOG(debug) << "[frameLoop] wait for frames: " << duration.count()
                          << "ms\n";
    }
    ...
    }
    {
      ...
      instance_latest_frames.timestamp =
    ...
    if (debug_enabled) {
      ...
      auto duration =
          std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
      VIAM_SDK_LOG(debug) << "[frameLoop] total:           " << duration.count()
                          << "ms\n";
    }
...
startPipeline(bool disableDepth, int depthWidth, int depthHeight,
  ...
  if (devices.size() == 0) {
    throw std::runtime_error(
        "no devices connected; please connect an Intel RealSense device");
  }
  ...
  for (auto &&dev : devices) {
    ...
    VIAM_SDK_LOG(info)
        << "found available RealSense device with serial number: "
        << current_serial_number;
    if (!target_serial_number.empty() &&
        current_serial_number == target_serial_number) {
      selected_device = dev;
  ...
  if (target_serial_number.empty()) {
    VIAM_SDK_LOG(info)
        << "no serial number specified in config, using first available device";
  ...
  VIAM_SDK_LOG(info) << "port:      "
                     << selected_device.get_info(RS2_CAMERA_INFO_PHYSICAL_PORT);
  ...
  if (!disableColor) {
    VIAM_SDK_LOG(info) << "color width and height from config: (" << colorWidth
                       << ", " << colorHeight << ")";
  ...
  if (!disableDepth) {
    VIAM_SDK_LOG(info) << "depth width and height from config: (" << depthWidth
                       << ", " << depthHeight << ")";
  ...
  auto fillProps = [](auto intrinsics,
                      std::string distortionModel) -> CameraProperties {
  ...
  if (!disableDepth) {
    ...
    VIAM_SDK_LOG(info) << "depth_width:    " << props.depth.width << alignedText
                       << " depth_height:   " << props.depth.height
                       << alignedText;
  }
...
void on_device_reconnect(rs2::event_information &info, rs2::pipeline pipeline,
  ...
  if (device == nullptr) {
    throw std::runtime_error("no device info to reconnect to. RealSense device "
  ...
  for (auto &&new_dev : info.get_new_devices()) {
    if (info.was_added(new_dev)) { // assures we're checking a device that was
                                   // part of the "added" event
      std::string new_dev_serial =
          new_dev.get_info(RS2_CAMERA_INFO_SERIAL_NUMBER);
      if (target_serial_number.empty()) {
        // No specific S/N configured, any new device is a candidate.
        VIAM_SDK_LOG(info)
            << "[on_device_reconnect] A new RealSense device (S/N: "
            << new_dev_serial
            << ") was connected. Attempting to use it as no specific S/N was "
               "configured.";
        ...
        if (new_dev_serial == target_serial_number) {
          VIAM_SDK_LOG(info)
              << "[on_device_reconnect] Configured device with S/N "
              << target_serial_number << " reconnected.";
          device_found_to_reconnect = true;
          break;
        }
      }
  ...
  VIAM_SDK_LOG(info)
      << "[on_device_reconnect] Device was reconnected, restarting pipeline";
  {
    // wait until frameLoop is stopped
  ...
  RealSenseProperties props;
  try {
    std::tie(pipeline, props) = startPipeline(
        device->disableDepth, device->depthWidth, device->depthHeight,
  ...
  std::thread cameraThread(frameLoop, pipeline, std::ref(ready), device,
                           props.depthScaleMm,
...
std::vector<std::string> validate(sdk::ResourceConfig cfg) {
  ...
  if (attrs.count("sensors")) {
    if (const sdk::ProtoList *sensors_list =
            attrs["sensors"].get<sdk::ProtoList>()) {
      if (sensors_list->empty()) {
        throw std::invalid_argument("sensors field cannot be empty, must list "
                                    "color and/or depth sensor");
      }
      ...
      throw std::invalid_argument(
          "'sensors' attribute must be a list of strings");
    }
  } else {
    throw std::invalid_argument(
...
int serve(int argc, char **argv) {
  std::shared_ptr<sdk::ModelRegistration> mr =
      std::make_shared<sdk::ModelRegistration>(
          sdk::API::get<sdk::Camera>(),
          sdk::Model{kAPINamespace, kAPIType, kAPISubtype},
          [](sdk::Dependencies deps,
