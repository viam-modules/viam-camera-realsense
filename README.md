...
This module depends on the [`librealsense` SDK](https://github.com/IntelRealSense/librealsense/releases). As of the time of writing, Ubuntu is the the only Linux Distro `librealsense` supports. We recommend adhering to the requirements of the underlying SDK of the Realsense and use Ubuntu when possible to avoid instability and unexpected behavior.
